<script lang="ts">
    ...
    if (AUTH_CONSTANTS.providers.huggingface)
        activeProviders.push({ id: 'huggingface', label: 'Sign in with Hugging Face', Icon: SiHuggingface });
    ...
    if (AUTH_CONSTANTS.providers.salesforce)
        activeProviders.push({ id: 'salesforce', label: 'Sign in with Salesforce', Icon: SiSalesforce });
