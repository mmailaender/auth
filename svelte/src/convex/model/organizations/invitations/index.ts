...
export const createInvitationModel = async (
  ctx: MutationCtx,
  args: { userId: Id<'users'>; email: string; role: 'role_organization_member' | 'role_organization_admin' | 'role_organization_owner' }
): Promise<{
  _id: Id<'invitations'>;
...
export const getInvitationsModel = async (
  ...
  Array<{
    ...
    email: string;
    role: string;
    invitedBy: { _id: Id<'users'> | undefined; name: string };
    expiresAt: number;
  }>
> => {
